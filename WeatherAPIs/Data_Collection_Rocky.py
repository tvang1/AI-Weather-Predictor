...
def get_open_weather(api,date,city,state):
    ...
    try: 
        if (city=="castle+rock" and state=="colorado"):
            ...
            with open(r"./API Outputs/castle_rock_data.csv","a+") as file1:
            ...
            lines_to_file = ( str(curr_line)+","+str(date)+","+str(a_dict['main']['temp'])+","+str(a_dict['main']['pressure'])+","+str(a_dict['weather'][0]['description'])+","+str(a_dict['wind']['speed'])+","+str(a_dict['wind']['deg']) )
            with open(r"./API Outputs/denver_data.csv","a+") as file1:
                file1.write(str(lines_to_file)+"\n")
            print ("Added to file successfully.")
            ...
            lines_to_file = ( str(curr_line)+","+str(date)+","+str(a_dict['main']['temp'])+","+str(a_dict['main']['pressure'])+","+str(a_dict['weather'][0]['description'])+","+str(a_dict['wind']['speed'])+","+str(a_dict['wind']['deg']) )
            with open(r"./API Outputs/boulder_data.csv","a+") as file1:
                file1.write(str(lines_to_file)+"\n")
            print ("Added to file successfully.")

    except:
        print ("An error log was saved to the API Outputs directory.")
        with open(r"./API Outputs/error_log.csv","w+") as file1:
...
def get_lines(name):
    if (name == "cr"):
        with open(r"./API Outputs/castle_rock_data.csv", 'r') as fp:
            num_lines = sum(1 for line in fp if line.rstrip())
            return num_lines
    elif (name == "dv"):
        with open(r"./API Outputs/denver_data.csv", 'r') as fp:
            num_lines = sum(1 for line in fp if line.rstrip())
            return num_lines
    elif (name == "bd"):
        with open(r"./API Outputs/boulder_data.csv", 'r') as fp:
            num_lines = sum(1 for line in fp if line.rstrip())
            return num_lines
