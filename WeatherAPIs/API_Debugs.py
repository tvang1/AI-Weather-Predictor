...
def get_open_weather(api,date,city,state):
    ...
    try: 
        if (city=="castle+rock" and state=="colorado"):
            ...
            with open(r"D:\GitRepositories\Simple-Weather-Predictor\WeatherAPIs\API Outputs\castle_rock_data-copy.csv","a+") as file1:
            ...
            lines_to_file = ( str(curr_line)+","+str(date)+","+str(a_dict['main']['temp'])+","+str(a_dict['main']['pressure'])+","+str(a_dict['weather'][0]['description'])+","+str(a_dict['wind']['speed'])+","+str(a_dict['wind']['deg']) )
            with open(r"D:\GitRepositories\Simple-Weather-Predictor\WeatherAPIs\API Outputs\denver_data-copy.csv","a+") as file1:
                file1.write(str(lines_to_file)+"\n")
            print ("Added to file successfully.")
            ...
            lines_to_file = ( str(curr_line)+","+str(date)+","+str(a_dict['main']['temp'])+","+str(a_dict['main']['pressure'])+","+str(a_dict['weather'][0]['description'])+","+str(a_dict['wind']['speed'])+","+str(a_dict['wind']['deg']) )
            with open(r"D:\GitRepositories\Simple-Weather-Predictor\WeatherAPIs\API Outputs\boulder_data-copy.csv","a+") as file1:
                file1.write(str(lines_to_file)+"\n")
            print ("Added to file successfully.")
...
def get_lines(name):
    if (name == "cr"):
        with open(r"D:\GitRepositories\Simple-Weather-Predictor\WeatherAPIs\API Outputs\castle_rock_data-copy.csv", 'r') as fp:
            num_lines = sum(1 for line in fp if line.rstrip())
            return num_lines
    elif (name == "dv"):
        with open(r"D:\GitRepositories\Simple-Weather-Predictor\WeatherAPIs\API Outputs\denver_data-copy.csv", 'r') as fp:
            num_lines = sum(1 for line in fp if line.rstrip())
            return num_lines
    elif (name == "bd"):
        with open(r"D:\GitRepositories\Simple-Weather-Predictor\WeatherAPIs\API Outputs\boulder_data-copy.csv", 'r') as fp:
            num_lines = sum(1 for line in fp if line.rstrip())
            return num_lines
